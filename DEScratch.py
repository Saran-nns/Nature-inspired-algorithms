...
def plantTypeCost(s, plant):
    ...
    elif (s > kwhPerPlant * maxPlants):
        return float(kwhPerPlant * maxPlants)
    else:
...
if __name__ == '__main__':
    ...
    best_revenue = 0.0
    best_genome = None
    max_profit = 1150000.0

    # while best_revenue <= 1150000.0:
            # print("425 Best genome and best revenue", best_genome, best_revenue)
    for i in range(200):
            # b donor selection
            ...
            # d selection
            print("416 Maximum revenue acheived so far", max(list_revenues))
            best_trial = selection(list_revenues)
            best_revenue = max(list_revenues)
            best_genome = best_trial
            # e update the population
            population = pop.update_population(best_trial)
            # print("418 The best genome", best_trial)
