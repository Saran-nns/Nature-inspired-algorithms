...
def initalize(benchmark):
    ...
    draw_route(shortest_path, tspmap)
...
def draw_route(shortest_path, tspmap):
    """
    Args:
    ...
    Returns:
        ...
        """
    # Convert the list of paths into list of lists with (CurrentCity, Nextcity)

    # _shortest_path = shortest_path.copy()
    _tspmap = tspmap.copy()
    map_paths = list(zip(shortest_path[:-1], shortest_path[1:]))

    # Get edge labels : Distance between the cities in tspmat
    edge_labels = np.zeros(len(map_paths))
    for i, tour in enumerate(map_paths):
        edge_labels[i] = _tspmap[tour[0]][tour[1]]

    # Combine map_paths and edge labels into dictionary
    path_map_dict = collections.OrderedDict(zip(map_paths,edge_labels.tolist()))

    map_paths_reduced = map_paths[:10]
    path_map__dict_reduced = take(10, path_map_dict.items())
